...
`vue-rx` 提供 `v-stream` 指令 你可以流式化 DOM 事件到一个 Rx Subject. 语法和 `v-on`相似, 指令参数对应事件名, 绑定值对应 Rx Subject.
...
### `v-stream`: 从子组件流式化自定义事件

跟流式化 `DOM` 事件很相似, `v-stream` 也可以被用于组件, 它会根据子组件触发的自定义事件创建 observables. 运作方式跟 `v-on` 相似:
...
你不能使用 `watch` 选项去观察订阅(subscriptions), 因为它在设置订阅之前就被处理完毕了. 但是你可以在 `created` 钩子中使用 `$watch` 作为替代方案.
